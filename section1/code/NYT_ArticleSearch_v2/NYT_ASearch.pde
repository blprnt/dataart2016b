...
int[] doASearchMonths(String q, int y, int startMonth, int endMonth) {
  int[] counts = new int[endMonth - startMonth + 1];
  int i = startMonth;
  while (i <= endMonth) {
    println(i);
    try {
      ASResult r = doASearch(q, y + "" + nf(i, 2) + "01", y + "" + nf(i, 2) + "31");
      println(r.hits);
      counts[i - startMonth] = r.hits;
      delay(queryDelay);
      i++;
    } catch (Exception e) {
     println("FAILED ON " + i + ". IF YOU SEE THIS MESSAGE A BUNCH OF TIMES IN A ROW, TRY AGAIN LATER, OR WITH A DIFFERENT QUERY. \n AND MAKE SURE YOU'VE ENTERED YOUR API KEY!"); 
...
ASResult doASearch(String q, String beginDate, String endDate) {
  ...
  println(getASURL(q, beginDate, endDate));
